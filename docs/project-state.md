...
- Для глюкозы уже добавлен отдельный график на странице `/glucose` с двумя линиями и без отдельного фильтра по датам.
...
- Начать реализацию dashboard-графиков, начиная с переиспользования glucose chart.
