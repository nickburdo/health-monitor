...
- Step 1 (local-first migration) is complete: the app has no server and no auth. All data lives in one IndexedDB database per browser (Dexie.js), mirroring `prisma/schema.prisma`.
- On first run with an empty database, the app seeds itself from `public/data/demo.json`, generated by `scripts/generate-demo-data.mjs`.
- Whole-database JSON export/import is available on the dashboard; CSV export is available per entity on its own page.
...
- Use Nuxt 4 plus Nuxt UI, running client-only (`ssr: false`).
- Store all data client-side in IndexedDB via Dexie.js; no server, no auth.
- `prisma/schema.prisma` is kept as documentation only — it is no longer connected to a live database, generator, or client.
